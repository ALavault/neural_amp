...
class AAFSSR(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if factor == 1 and aa_latency_samples != 16:
            raise ValueError("custom AA latency applies only to oversampled modes")
        if factor > 1 and not 1 <= aa_latency_samples <= 48:
            raise ValueError("oversampled AA latency must be between 1 and 48 samples")
        self.processor = (
            FullRateIsland(
                branch,
                factor=factor,
                latency_samples=aa_latency_samples,
            )
            if factor > 1
            else branch
        )
        ...
        if factor > 1 and aa_latency_samples != 16:
            self.native_format = "fssr-quality-aa-native-v1"
            self.native_campaign_version = "FSSR-QUALITY-AA-v2"
