...
def harmonic_fidelity_guard(
    ...
    *,
    ...
    candidate_complex_error = normalized_error(
        candidate_harmonics, reference_harmonics
    )
...
def known_reference_alias_residual(
    ...
    *,
    ...
    reference_energy = float(
        np.sum(np.abs(reference_spectrum[harmonic_bins]) ** 2)
    )
...
def fundamental_delay_samples(
    ...
    *,
    ...
    if abs(candidate_bin) <= np.finfo(float).tiny or abs(reference_bin) <= np.finfo(
        float
    ).tiny:
