#!/usr/bin/env python3
"""Run the preregistered M4 spline-grid diagnostic (S3 t33, knots over +-0.4)."""
...
def run_id(config: dict, device: str, model: str, seed: int, preflight: bool) -> str:
    stage = "m4_grid_preflight" if preflight else "m4_grid"
...
def launch(model: str, device: str, seed: int, run: str, preflight: bool) -> None:
    ...
    if run_dir.exists():
        ...
        if status["status"] != "completed":
            raise RuntimeError(f"existing grid run failed: {run}")
...
def main() -> None:
    ...
    if args.devices is not None:
        if any(device not in config["devices"] for device in args.devices):
            raise ValueError("device outside the preregistered diagnostic")
    ...
    if any(seed not in config["seeds"] for seed in seeds):
        raise ValueError("seed outside the preregistered diagnostic")
    for model in args.models:
