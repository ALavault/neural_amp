...
def full_rate_island(
    ...
    *,
    ...
    branch = apply_r2_fixture(
        fixture, interpolated, SAMPLE_RATE_HZ * factor
    ).output
...
def synthetic_reference(
    ...
    high_output = apply_r2_fixture(
        fixture, high_input, SAMPLE_RATE_HZ * factor
    ).output
        ...
        for mode in modes:
            ...
            for k0 in K0_VALUES:
                for amplitude in AMPLITUDES:
                    ...
                    asr = dc_separated_asr(
                        candidate, k0=k0, floor_db=floor_db
                    )
                    off_asr = dc_separated_asr(
                        aligned_off, k0=k0, floor_db=floor_db
                    )
            ...
            rows.append(
                {
                    ...
                    "paired_asr_gain": aggregate_grid(
                        conditions, "paired_asr_gain_db"
                    ),
