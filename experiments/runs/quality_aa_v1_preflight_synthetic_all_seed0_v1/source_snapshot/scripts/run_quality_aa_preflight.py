...
from fssr_nam.campaign.quality_aa_gates import evaluate_preflight_gate
...
from fssr_nam.campaign.quality_aa_registry import (
    append_gate_event,
    gate_decisions,
)
from fssr_nam.campaign.quality_aa_v1 import (
    CAMPAIGN_VERSION,
    make_run_id,
...
SUMMARY_PATH = ROOT / "experiments/summaries/quality_aa_v1/preflight.json"
...
def main() -> int:
    ...
    if "preflight" in decisions:
        raise RuntimeError("QUALITY-AA preflight is already recorded")
    if (
        RUN_DIR.exists()
        or SUMMARY_PATH.exists()
        or any(run["run_id"] == RUN_ID for run in read_runs(GLOBAL_LEDGER))
    ):
        raise RuntimeError("QUALITY-AA preflight run ID is already reserved")
