...
def _mechanism_rows(evidence: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    if evidence.get("campaign_version") != CAMPAIGN_VERSION:
    ...
    for route in ROUTES:
        ...
        for fixture in R2_FIXTURES:
            ...
            for condition in row["conditions"]:
                ...
                measured_asr.append(
                    _finite(condition.get("asr_linear"), "linear ASR")
                )
...
def final_backend_decision(
    ...
    if not admissible:
        return {
            ...
            "verdict": "NO-GO-QUALITY-AA-v1",
            "selected_backend": None,
            "reason": "no route passed mechanism, parity, latency, and runtime",
        }
    qualities = {
        route: float(
            mechanism_gate["routes"][route][
                "median_known_reference_alias_residual_db"
            ]
        )
    ...
    selected = min(
        contenders,
        key=lambda route: native_gate["routes"][route][
            "block64_p95_ns_per_sample"
        ],
    )
    return {
        "campaign_version": CAMPAIGN_VERSION,
        "verdict": "GO-QUALITY-AA-v1",
