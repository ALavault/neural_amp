...
QUALITY_AA_FORMAT = "fssr-quality-aa-native-v1"
QUALITY_AA_CAMPAIGN_VERSION = "FSSR-QUALITY-AA-v2"
...
def validate_r2_native_payload(payload: Mapping[str, Any]) -> None:
    ...
    native_format = payload.get("format")
    campaign_version = payload.get("campaign_version")
    if (native_format, campaign_version) not in {
        (FORMAT, CAMPAIGN_VERSION),
        (QUALITY_AA_FORMAT, QUALITY_AA_CAMPAIGN_VERSION),
    }:
        raise ValueError("R2/QUALITY-AA format and campaign version are inconsistent")
    quality_aa = native_format == QUALITY_AA_FORMAT
    required = {
        "version": 1,
        "precision": "float32",
    ...
    expected_latency = 1 if aa_mode == "adaa1" else 0
    if factor > 1:
        valid_latency = (
            isinstance(latency, int)
            and not isinstance(latency, bool)
            and 1 <= latency <= 48
            and (quality_aa or latency == 16)
        )
    else:
        valid_latency = latency == expected_latency
    ...
    if factor == 1:
        ...
        if resampling.get("linear_delay_samples") != latency:
            raise ValueError("full island delay differs from model latency")
        taps = factor * int(latency) + 1
...
def _model_sizes(model: Any, payload: Mapping[str, Any]) -> dict[str, int]:
    ...
    if payload["family"] == "aa-nam":
        ...
        if factor > 1:
            ...
            taps = len(resampling["upsample_coefficients"])
            persistent += 2 * ((taps - 1) * 4 + pointer_bytes)
            persistent += int(payload["latency_samples"]) * 4 + pointer_bytes
        return {
...
def _resampling_payload(processor: Any, factor: int) -> dict[str, Any] | None:
    ...
    return {
        "factor": factor,
        "linear_delay_samples": int(processor.latency_samples),
...
def build_r2_native_payload(model: Any) -> dict[str, Any]:
    ...
    native_format = getattr(model, "native_format", FORMAT)
    campaign_version = getattr(model, "native_campaign_version", CAMPAIGN_VERSION)
