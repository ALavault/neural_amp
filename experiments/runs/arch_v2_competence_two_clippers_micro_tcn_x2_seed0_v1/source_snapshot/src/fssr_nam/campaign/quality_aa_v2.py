...
def resolve_protocol(root: Path) -> dict[str, Any]:
    ...
    resolved["selection"]["valid_verdicts"] = overlay["valid_terminal_verdicts"]
...
def validate_repository_configs(root: Path) -> dict[str, Any]:
    ...
    if active not in {CAMPAIGN_KEY, "amp_quality_arch_v1"}:
    ...
    if active == "amp_quality_arch_v1":
        descendant = lineages.get("lineages", {}).get(active, {})
        _require_equal(descendant.get("parent"), CAMPAIGN_KEY, "descendant parent")
        _require_equal(
            entry.get("historical_artifacts_immutable"),
            True,
